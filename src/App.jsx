...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (hash === "#thanks") {
      ...
      const isAllowedReferrer = allowedReferrers.some((allowedRef) =>
      ...
      if (status === "successful" && transactionId && isAllowedReferrer) {
        ...
        console.error("Failed to meet criteria for opening modal");
      }
    }
  }, []);
    ...
    <>
      ...
      {activeModal === "growth" && (
        <DownloadModal
          isOpen={true}
          onClose={closeModal}
          title="Personal Growth Download"
          fileName="growth.pdf"
        />
      )}

      {activeModal === "thankYou" && (
        <ThankYouModal isOpen={true} onClose={closeModal} />
      )}
    </>
  );
};
